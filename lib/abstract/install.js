module.exports = function (version, done) {
  ...
  async.waterfall([function (callback) {
    var exists = fs.existsSync(dirname.truffle + 'v' + version);
    if (exists) {
    ...
    if (versions.indexOf(version) === -1) {
      ...
      console.log("Unknown truffle compiler version: '" + version + "'.");
    } else {
    ...
    var file_name = dirname.src + 'v' + version + '/truffle-' + version + '.tgz';
    ...
    var new_path = path + 'truffle-' + version;
    fs.renameSync(old_path, new_path);
    old_path = path + 'truffle-' + version;
    new_path = dirname.truffle + 'v' + version;
  ...
  }, function (new_path, callback) {
    var command = 'cd ' + new_path + '/ && npm install --production';
    var npmInstall = child_process.exec(command, function (error, stdout) {
      if (error) {
        console.log(error);
      } else {
        callback(null);
